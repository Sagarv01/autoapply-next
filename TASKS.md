...
- [~] M-A Onboarding wizard gating the bot. **Logic foundation DONE (tested):**
  ...
  - [~] M-A.4 Qt UI. **Threading pattern + sign-in DONE:**
    ...
    - [ ] NEXT: 19-field profile form (CANDIDATE_FIELDS; conditional visa when not citizen/PR; EEO defaults; save via runner.submit(save_facts)); documents upload (resume→assets/'SAGAR VERMA.docx', cover); criteria screen (search.skills+location); acknowledge screen (HONESTY_LINE from ui/run_status); review. Then the GATING controller: MainWindow routes to first_incomplete_step(signed_in=auth.signed_in, acknowledged=flag) on start + after each step, locks the bot screens until is_onboarding_complete. Paywall upgrade UI (checkout_flow.run_checkout via runner.submit_coro + QDesktopServices.openUrl). "Waiting on you" held UI.
