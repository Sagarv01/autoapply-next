...
- [~] M-A Onboarding wizard gating the bot. **Logic foundation DONE (tested):**
  ...
  - [~] M-A.4 Qt UI. **Threading pattern + sign-in DONE:**
    ...
    - [x] Free apply tailoring: **DONE 2026-06-16 (commit 52f90dd).** engine/tailoring_policy (allows_tailoring Pro-only + module gate, default True back-compat); adapter._produce_documents branches Pro->tailorer.tailor(full) vs non-Pro->base resume+cover PDFs (no LLM); worker._refresh_tailoring_policy fetches tier before each apply/batch (3 runners) and sets the gate, leaves unchanged on fetch failure. 403->upgrade remains the safety net. 16 tests. LibreOffice base-cover conversion is manual-verify.
...
- [~] M-F Packaging mac+win + version check. **BUILD-COMPLETE:** spec is cross-platform + collect_submodules bundles every new module; mac .app (smoke-tested) + Windows onedir+exe (same spec, build on a Win box); in-app version floor DONE (version_check + UpdateRequiredScreen + startup /api/config check, walls below floor) commit 95a8172. **BLOCKED:** code signing + installer wrapping (Apple notarization cert + Windows EV cert; packaging/windows/sign.ps1 + CERT_CHECKLIST.md ready). Build steps in HANDOFF.
