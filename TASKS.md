...
- [~] M-A Onboarding wizard gating the bot. **Logic foundation DONE (tested):**
  ...
  - [~] M-A.4 Qt UI. **Threading pattern + sign-in DONE:**
    ...
    - [ ] REMAINING UI: wire BillingScreen into MainWindow (+ open on 403 needs_pro); "Waiting on you" held-queue UI (HeldQueue.pending list + answer field); render RunState/cooldown copy in the run view (engine/run_status). M-C.3 UI tail: answer-unblocks re-apply (flip jobs.db 'held'→'queued'; needs job_id→URL), Resend nudge (Resend ABSENT in proxy). Humanize pass on ui/run_status.py + screen draft strings.
