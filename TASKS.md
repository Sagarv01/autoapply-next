...
- [~] M-A Onboarding wizard gating the bot. **Logic foundation DONE (tested):**
  ...
  - [ ] M-A.4 Qt UI: real sign-in screen (AuthManager-driven), 19-field profile form (from CANDIDATE_FIELDS), resume+cover upload, criteria screen, honesty line, review; wire the wizard to gate main_window via `is_onboarding_complete`. (UI layer wraps the tested models above.)
- [~] M-C Unknown-question held queue. **Logic core DONE (tested, 42 tests):**
  ...
  - [ ] M-C.3 INTEGRATION (pairs w/ UI batch): adapter catches QuestionHeldError → 'held' status (not 'failed', not a circuit failure); worker signal; "Waiting on you" UI badge+prompt; answer-unblocks re-apply if listing live; Resend nudge (Resend absent in proxy).
- [ ] M-D Run UX states + time-of-day pacing + invisible 100/day cap + cooldown copy + keep-polling.
