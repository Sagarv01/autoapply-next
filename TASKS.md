...
- [~] M-A Onboarding wizard gating the bot. **Logic foundation DONE (tested):**
  ...
  - [~] M-A.4 Qt UI. **Threading pattern + sign-in DONE:**
    ...
    - [ ] Free apply tailoring: **DECIDED 2026-06-16 — SKIP tailoring for non-Pro; Free/Basic apply with BASE docs (no 403).** Implementation DEFERRED (user chose Phase F next). To build: in the engine integration, fetch the user's tier (proxy_billing.fetch_subscription_status) and, for non-Pro, do NOT install the tailorer's task="tailor" path / use the base resume+cover directly; the 403/upgrade only appears if tailoring is explicitly invoked.
